...
import {getHTTPAddress, postHTTPAddres} from '../utils/additionalFunctions';
...
function Registration() {
    ...
    const handleRegister = () => {
        ...
        if (secPassword === password) {
            getHTTPAddress(
                ...
                if (users.length === 0) {
                    postHTTPAddres('http://localhost:5000/users', user);
                    navigate('/login');
                } else {
                    resetData();
                }
    ...
    return (
        <div className="flex flex-col items-center">
            <div className="flex flex-col gap-2 w-fit m-auto mt-8 p-1 shadow-lg">
                <input
                    placeholder="e-mail"
                    className={
                        valid
            ...
            >
                {error === 'log'
                    ...
                    : 'Unexpected error. Check equality of both passwords'}
            </div>
        </div>
    );
}
